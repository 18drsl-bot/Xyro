...
checkForFile("me/prefix.txt", "!")
if isfile("me/prefix.txt") then
	local ok, saved = pcall(readfile, "me/prefix.txt")
	_G.prefix = (ok and saved and saved ~= "") and saved or "!"
else
	_G.prefix = "!"
end
...
make("UIGradient", {
	Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, COL.accent),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160, 105, 255)),
...
H.makeDraggable = function(frame, handle, conn)
	...
	conn(UIS.InputEnded, function(i)
		if isDrag(i) then
			dragging = false
			if H.saveWindowPosition then
				pcall(H.saveWindowPosition, frame)
			end
		end
	end)
	return frame
...
H.addDropShadow = function(target, size, opacity)
	pcall(function()
		size = size or 24
		opacity = opacity or 0.35
		local img = H.make("ImageLabel", {
			Name = "DropShadow",
			BackgroundTransparency = 1,
...
connect(hbBox.FocusLost, function()
	local n = H.clampV(tonumber(hbBox.Text) or hitboxSize, 1, 10)
	hitboxSize = n
...
H.WINDOW_POSITIONS = WINDOW_POSITIONS
...
local function applyConfig(cfg)
	...
	if type(cfg.windowPositions) == "table" then
		for name, p in pairs(cfg.windowPositions) do
			if type(p) == "table" and tonumber(p.x) and tonumber(p.y) then
				WINDOW_POSITIONS[name] = { x = tonumber(p.x), y = tonumber(p.y) }
			end
		end
		applyWindowPositions()
	end
	if type(cfg.lastTab) == "string" then
		H.lastTabName = cfg.lastTab
	end
...
H.addDropShadow(setFrame, 26, 0.4)
...
local function saveWindowPosition(frame)
	if frame and frame.Name and frame.Position then
		WINDOW_POSITIONS[frame.Name] = { x = frame.Position.X.Offset, y = frame.Position.Y.Offset }
	end
end
H.saveWindowPosition = saveWindowPosition

local function applyWindowPositions()
	for name, p in pairs(WINDOW_POSITIONS) do
		local f = gui:FindFirstChild(name)
		if f and f:IsA("GuiObject") and f.Position.X.Scale == 0 and f.Position.Y.Scale == 0 then
			f.Position = UDim2.new(0, p.x or 0, 0, p.y or 0)
		end
	end
end
...
H.addDropShadow(picker.frame, 24, 0.4)
...
-- Website nametags: rules live in nametags.json in the script repo and are
-- fetched at runtime, so tags can be edited on github.com without re-executing.
-- Website nametags: rules live in nametags.json in the public Xyro repo,
...
local function openHelp(filter)
	local rows = {}
	local seen = {}
	local needle = (filter or ""):lower():gsub("^%s+", ""):gsub("%s+$", "")
	local function emit(group)
		local first = true
		for _, s in ipairs(ORDER) do
			if s.group == group and not s.debug then
				local hay = (s.name .. " " .. table.concat(s.alias or {}, " ") .. " " .. s.help):lower()
	...
	if needle ~= "" and #rows == 0 then
		rows[#rows + 1] = { text = "no commands match '" .. filter .. "'", header = true }
	end
	listWindow("HelpUI", "Vert's hub", rows)
end

local helpHookInstalled = false
local function installHelpSearch()
	if helpHookInstalled then
		return
	end
	helpHookInstalled = true
	local oldListWindow = listWindow
	listWindow = function(name, title, rows)
		oldListWindow(name, title, rows)
		if name ~= "HelpUI" then
			return
		end
		local win = gui:FindFirstChild("HelpUI")
		local sc = win and win:FindFirstChildOfClass("ScrollingFrame")
		if win and sc then
			...
			round(search, 6)
			sc.Position = UDim2.new(0, 10, 0, 66)
			sc.Size = UDim2.new(1, -20, 1, -76)
			connect(search.FocusLost, function(enter)
				if enter then
					local existing = gui:FindFirstChild("HelpUI")
					if existing then
						existing:Destroy()
					end
					openHelp(search.Text)
				end
			end)
		end
	end
...
H.addDropShadow(main, 26, 0.4)

selectTab(H.lastTabName or "Speed")
...
_G.VertChatRemotes = _G.VertChatRemotes or {}
do
						...
						if type(msg) == "string" and msg:sub(1, 1) == _G.prefix then
							...
							elseif method == "FireServer" then
								if self.Name == "SayMessageRequest" then
									...
									for _, rn in ipairs(_G.VertChatRemotes) do
